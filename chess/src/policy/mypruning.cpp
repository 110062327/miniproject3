...
#include "./mypruning.hpp"
/**
 * @brief Policy class for random policy, 
 * your policy class should have get_move method
 */
int _heuristicvalue(State mystate){
...
function alphabeta(node, depth, α, β, maximizingPlayer) is
    ...
    if maximizingPlayer then
        ...
        return value
*/
int alphabeta(State& node, int depth,int a,int b, bool maximizingPlayer){
  if(depth==0||std::abs(_heuristicvalue(node))>=50){
    return _heuristicvalue(node);
  }
  if(maximizingPlayer){
    int value=-1e9;
    for(auto child:node.legal_actions){
      State* next_move=node.next_state(child);
      value = std::max(value, alphabeta(*next_move, depth-1,a,b, false));
      a=std::max(a,value);
      if(value>=b){
        break;
      }
    ...
    for(auto child:node.legal_actions){
      State* next_move=node.next_state(child);
      value = std::min(value, alphabeta(*next_move, depth-1,a,b, true));
      b=std::min(b,value);
      if(value<=a){
        break;
      }
 ...
 * @return Move 
 */
Move _myclass::get_move(State *state, int depth){
  ...
  if(state->player==0){//white
    ...
    for(auto child:state->legal_actions){
      ...
      int tmp=alphabeta(*next_state, depth,-1e9,1e9, false);
      if(tmp>value){
        value=tmp;
        ans=child;
      }
